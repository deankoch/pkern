...
# project directory
dir_project = 'D:/pkern/vignettes/comparisons'
...
# packages tested (pick any subset of the 'pkern', 'fields', 'RandomFields', 'geoR', 'gstat')
pkg_test = c('pkern', 'fields', 'RandomFields', 'geoR', 'gstat')
...
treed_pts = pkern_coords(treed, out='sf')[idx_sub,]
...
#'
#' For the treed example itself, we use `pkern_rescale` to generate up-scaled versions where points are
...
# use pkern to up-scale and down-scale
treed_up = lapply(up_test, function(up) pkern_rescale(treed, up=up))
treed_down = lapply(down_test, function(down) pkern_rescale(treed, down=down))

# export to a big list of terra SpatRasters
eg_rast = lapply(c(treed_up, treed_down), pkern_export)
...
n_max = list(
  ...
  ozone = list(fit=c(pkern=Inf, fields=Inf, RandomFields=Inf, geoR=Inf, gstat=Inf),
               pred=c(pkern=Inf, fields=Inf, RandomFields=Inf, geoR=Inf, gstat=10^7),
               var=c(pkern=Inf, fields=Inf, RandomFields=Inf, geoR=Inf, gstat=10^7)),

  meuse = list(fit=c(pkern=Inf, fields=Inf, RandomFields=Inf, geoR=Inf, gstat=Inf),
               pred=c(pkern=Inf, fields=Inf, RandomFields=10^7, geoR=10^7, gstat=10^7),
               var=c(pkern=10^7, fields=10^7, RandomFields=10^7, geoR=10^7, gstat=10^7)),

  treed = list(fit=c(pkern=Inf, fields=Inf, RandomFields=Inf, geoR=Inf, gstat=Inf),
               pred=c(pkern=Inf, fields=10^7, RandomFields=10^6, geoR=10^6, gstat=10^6),
               var=c(pkern=10^6.5, fields=10^6, RandomFields=10^6, geoR=10^6, gstat=10^6)),

  # raster examples

  treed_88 = list(fit=c(pkern=Inf, fields=Inf, RandomFields=Inf, geoR=Inf, gstat=Inf),
                  pred=c(pkern=Inf, fields=Inf, RandomFields=10^7, geoR=10^7, gstat=10^7),
                  var=c(pkern=10^7, fields=10^7, RandomFields=10^7, geoR=10^7, gstat=10^7)),

  treed_352 = list(fit=c(pkern=Inf, fields=Inf, RandomFields=Inf, geoR=Inf, gstat=Inf),
                   pred=c(pkern=Inf, fields=10^7, RandomFields=10^6.5, geoR=10^6.5, gstat=10^6.5),
                   var=c(pkern=10^7, fields=10^6.5, RandomFields=10^6.5, geoR=10^6.5, gstat=10^6.5)),

  treed_1376 = list(fit=c(pkern=Inf, fields=Inf, RandomFields=Inf, geoR=Inf, gstat=Inf),
                    pred=c(pkern=Inf, fields=10^6.5, RandomFields=10^5, geoR=10^5, gstat=10^5),
                    var=c(pkern=10^6.5, fields=10^5, RandomFields=10^5, geoR=10^5, gstat=10^5)),

  # these are extras where we only want the fitting times

  treed_5440 = list(fit=c(pkern=Inf, fields=0, RandomFields=0, geoR=0, gstat=0),
                    pred=c(pkern=0, fields=0, RandomFields=0, geoR=0, gstat=0),
                    var=c(pkern=0, fields=0, RandomFields=0, geoR=0, gstat=0)),

  treed_21632 = list(fit=c(pkern=Inf, fields=0, RandomFields=0, geoR=0, gstat=0),
                     pred=c(pkern=0, fields=0, RandomFields=0, geoR=0, gstat=0),
                     var=c(pkern=0, fields=0, RandomFields=0, geoR=0, gstat=0))
)
